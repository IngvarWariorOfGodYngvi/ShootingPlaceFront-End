              ...
              <q-item-section>
                ...
                <q-item-label v-else>NIE</q-item-label>
                  <q-item-label v-if="!members.license.isValid&&members.weaponPermission">POSIADA NIE WAŻNĄ LICENCJĘ!!!</q-item-label>
                   <q-item-label v-if="members.license.isValid&&members.weaponPermission" >Posiada ważną licencję</q-item-label>
                  <q-expansion-item label="rozwiń">
                  <q-item-label v-if="!members.weaponPermission" >Dodaj pozwolenie</q-item-label>
...
export default {
  ...
  methods: {
    ...
    getListMembers () {
      fetch('http://localhost:8080/member/activelist?active=true&adult=true', {
        method: 'GET'
