            ...
            <q-select class="full-width rounded text-positive"
                  ...
                  <template v-slot:option="option">
                    <q-item class="rounded" dense style="padding: 0; margin: 0;" v-bind="option['itemProps']" v-on="option.itemEvents" >
                      ...
                      <q-item-section v-else style="padding: 0.5em; margin: 0;" class="bg-warning rounded" @click="memberName = option.opt.secondName+' '+option.opt.firstName+' '+ option.opt.legitimationNumber;temp = option.opt.legitimationNumber">
...
<!--          </q-item>-->
    ...
    <Member v-if="temp!=null" :member-number-legitimation="temp"></Member>
      ...
      :virtual-scroll-sticky-size-end="32" dense :items="memberDTOArgRearrangeTable"  type="table" visible class="row full-width q-pa-none round bg-dark" style="height: 90vh;">
    ...
    <template v-slot="{ item, index }">
      <tr v-if="!item.erased" :key="index" class="rounded bg-dark text-positive" style="cursor:pointer" @click="showloading();allMember=false;memberName =item.secondName + ' '+item.firstName+' leg. '+item.legitimationNumber;temp = item.legitimationNumber;">
        ...
        <td style="width:5%;" :class="item.pzss?'bg-green-3 text-center text-black':'bg-warning text-center text-black'"><q-icon :name="item.pzss?'done':'cancel'" :color="item.pzss?'':'primary'" size="1rem"/></td>
        <td style="width:5%;" :class="item.image!=null?'bg-green-3 text-center text-black':'text-center text-black'"><q-icon :name="item.image!=null?'done':'cancel'" :color="item.image!=null?'':'primary'" size="1rem"/></td>
        <td style="width:10%;" class="text-center text-bold">{{item.legitimationNumber}}</td>
        <td style="width:15%;" v-if="item.license.number!=null && item.license.valid" class="bg-green-3 text-center text-black"><q-tooltip anchor="top middle" :offset="[35, 35]" content-class="text-body1 bg-secondary">Aktualna</q-tooltip><div>{{item.license.number}}</div></td>
        <td style="width:15%;" v-if="item.license.number!=null && !item.license.valid" class="bg-warning text-center text-black"><q-tooltip anchor="top middle" :offset="[35, 35]" content-class="text-body1 bg-secondary">Nieaktualna - ważna do: {{convertDate(item.license.validThru)}}</q-tooltip><div>{{item.license.number}}</div></td>
