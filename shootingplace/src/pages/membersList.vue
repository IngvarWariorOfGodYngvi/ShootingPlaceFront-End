<template>
  ...
  <div>
            ...
            <q-select dense class="full-width rounded text-white" :label="memberName" label-color="white" input-style="color: white;" bg-color="primary" use-input filled hide-selected :options="options" @filter="filter" @input="allMember = false" :value="options">
                  <template v-slot:option="option">
                    <q-item class="rounded" dense style="padding: 0px; margin: 0px;" v-bind="option.itemProps" v-on="option.itemEvents" >
                      <q-item-section v-if="option.opt.active" style="padding: 0.5em; margin: 0px;" @click="memberName = option.opt.secondName+' '+option.opt.firstName+' '+ option.opt.legitimationNumber,temp = option.opt.legitimationNumber,temp = option.opt.legitimationNumber">
                      ...
                      <q-item-section v-else style="padding: 0.5em; margin: 0px;" class="bg-red-2 rounded" @click="memberName = option.opt.secondName+' '+option.opt.firstName+' '+ option.opt.legitimationNumber,temp = option.opt.legitimationNumber">
    ...
    <template v-slot="{ item, index }">
      <tr v-if="!item.erased" :key="index" dense class="rounded" style="cursor:pointer" @click="showloading(),allMember=false,memberName =item.secondName + ' '+item.firstName+' leg. '+item.legitimationNumber, temp = item.legitimationNumber">
      ...
      <tr v-if="item.erased" :key="index" dense style="cursor:pointer" class="full-width rounded" @click="showloading(),allMember=false,memberName =item.secondName + ' '+item.firstName+' leg. '+item.legitimationNumber,temp = item.legitimationNumber">
        <td style="width:25%;" class="xyz"><b>{{index +1}}</b> {{item.secondName}} {{item.firstName}}</td>
        <td style="width:10%;" class="text-center">{{convertDate(item.joinDate)}}</td>
        <td style="width:10%;" class="text-center text-bold">{{item.legitimationNumber}}</td>
