  ...
  <q-page padding>
    ...
    <q-expansion-item label="Licencje nieważne" dense class="text-left text-h6 text-bold bg-grey-3" group="list">
      <q-card class="text-body2">
        <div class="row">
          <div class="q-pa-md text-left col full-width no-outline text-h5 text-bold">Ilość osób {{ members2.length }}
          </div>
        ...
        <q-virtual-scroll :items="members2" virtual-scroll-slice-size="100" style="height: 50vh">
          <template v-slot="{ item, index }">
            <div :key="index" class="row">
              <q-checkbox dense v-if="item.license.paid" v-model="licenseList" :val="item.uuid" left-label>
                {{ index + 1 }}.
              </q-checkbox>
              <q-checkbox dense color="grey" v-else disable left-label>{{ index + 1 }}.</q-checkbox>
              <div class="col" @dblclick="number = item.legitimationNumber,memberDial=true">
    ...
    <q-expansion-item label="Licencje ważne" dense class="text-left text-h6 text-bold bg-grey-3" group="list">
      <q-card class="text-body2">
        <div class="row">
          <div class="q-pa-md text-left col full-width no-outline text-h5 text-bold">Ilość osób {{ members.length }}
          </div>
          <div v-if="licenseList.length>0" class="q-pa-md text-right">
            <q-btn @click="memberName='WSZYSCY - którzy mają zaznaczone opłacono w PZSS', prolongLicenseAlertAll=true"
            ...
            <div :key="index" dense class="row">
              ...
              <div class="col" @dblclick="number = item.legitimationNumber,memberDial=true">
                ...
                <q-field dense class="col" label="Nazwisko i Imię" standout="bg-accent text-black" stack-label>
                  <div @dblclick="number = item.legitimationNumber,memberDial=true">
                    <div class="self-center col full-width no-outline row text-black" tabindex="1">{{ item.secondName }}
                      {{ item.firstName }}
                    </div>
                  </div>
      ...
      <q-card class="text-body2">
        <div class="row">
          <div class="q-pa-md text-left col full-width no-outline text-h5 text-bold">Ilość osób
            {{ quantitySumLicensePayment.length }}
          </div>
      ...
      <q-card>
        ...
        <q-card-actions align="right">
          ...
          <q-btn flat label="Opłać" color="primary" v-close-popup
                 @click="showloading (),addLicenseHistoryPayment (memberUUID)"/>
        </q-card-actions>
      </q-card>
    </q-dialog>
      ...
      <q-card>
        ...
        <q-card-actions align="right">
          ...
          <q-btn flat label="Przedłuż" color="primary" v-close-popup
                 @click="showloading (),prolongLicense (memberUUID, licensePistolPermission, licenseRiflePermission, licenseShotgunPermission)"/>
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="memberDial" style="min-width: 80vw">
      <q-card style="min-width: 80vw">
        <q-card-section class="flex-center">
          <Member :member-number-legitimation="number"></Member>
        </q-card-section>
