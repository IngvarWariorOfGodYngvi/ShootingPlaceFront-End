  ...
  <div>
    <div :class="mobile ? 'col' : 'row'">
      <q-card-section class="col-3">
            ...
            <template v-slot:append>
              <q-icon name="event" color="positive" class="cursor-pointer">
                <q-popup-proxy ref="qDateProxy" transition-show="scale" transition-hide="scale">
                  <q-date v-model="firstDate" @input="getJudgingList(firstDate, secondDate)" mask="YYYY-MM-DD"
        ...
        <q-item class="col">
          <q-input class="full-width" color="positive" input-class="text-positive" label-color="positive" dense filled
            ...
            <template v-slot:append>
              <q-icon name="event" color="positive" class="cursor-pointer">
                <q-popup-proxy ref="qDateProxy" transition-show="scale" transition-hide="scale">
                  <q-date v-model="secondDate" @input="getJudgingList(firstDate, secondDate)" mask="YYYY-MM-DD"
                    class="bg-dark text-positive">
                    <div class="row items-center justify-end">
                      <q-btn v-close-popup label="Zamknij" color="primary" />
                    </div>
        ...
        <div class="q-pa-md">
          <q-btn color="primary" text-color="positive" @click="getJudgingList(firstDate, secondDate)"
            label="Wyszukaj"></q-btn>
          <p></p>
    <q-btn color="primary" label="pobierz Raport Sędziowania" @click=" showloading(); getJudgingReport(firstDate, secondDate) " />
          <!-- <q-btn v-if="firstDate != null && secondDate != null" @click="getSumXLSXFile()" label="pobierz plik xlsx"
            color="green-3" text-color="black"></q-btn> -->
      ...
      <q-card-section class="col">
        ...
        <div v-if="quantitySum.length > 0"
          class="q-pa-md self-center col full-width no-outline text-bold text-center text-h6">Ilość Wpisów :
          {{ quantitySum.length }}
        </div>
        <q-virtual-scroll v-if="quantitySum.length > 0" :items="quantitySum" type="table" dense class="row full-width"
          style="height: 50vh;">
          <template v-slot:before>
            <thead class="thead-sticky text-left">
              <tr>
                <th class="text-left">
                  <div class="row">
                    <div style="width: 2vw">{{ quantitySum.length }}</div> Nazwisko i Imię
                  </div>
                </th>
                <th>
                  <div>Funkcja</div>
          ...
          <template v-slot="{ item, index }">
            <tr :key="index" class="rounded" style="cursor:pointer"
              ...
              <td class="text-left xyz row">
                <div style="width: 1.5vw"><b>{{index +1}}</b></div> {{ item.secondName }} {{ item.firstName }}
              </td>
