<template>
  <div>
    <q-btn glossy v-if="AddGroupAmmoExp" label="Dodaj Amunicję" :class="mobile ? 'col-6' : 'col-3'" color="primary" @click="open = !open" />
      ...
      <q-card class="bg-dark text-positive" style="min-width: 60vw;">
        <div class="row">
          <q-select label="Wybierz osobę z Klubu" popup-content-class="bg-dark text-positive"
            ...
            @input="otherName = Object({ secondName: '0', firstName: '0', id: '0' })" @filter="filterFn"
            class="col q-ma-xs">
          ...
          <q-select @popup-show="getOther()" @popup-hide="getOther()" options-dense class="col q-ma-xs" dense filled
            ...
            emit-value map-options :class="otherName.secondName !== '0' ? 'bg-primary' : 'bg-secondary'"
            :options="options1"
        ...
        <q-item dense v-for="(item, uuid) in calibers" :key="uuid" :val="item.uuid" style="">
          ...
          <q-input v-model="item.counter" class="col" color="positive" label-color="positive"
            @focus="item.counter.startsWith('0') ? item.counter = '' : item.counter = item.counter"
            input-class="text-positive" dense filled :label="'max: ' + item.quantity"
            @input="item.counter === '' ? item.counter = '0' : item.counter = item.counter; onInput(item, item.counter)"
            @keypress.enter="dis = true, simulateProgress(memberName.legitimationNumber, otherName.id)"
            onkeypress="return (event.charCode > 44 && event.charCode < 58)" type="number"
            :max="item.quantity"></q-input>
        ...
        <q-item dense>
          ...
          <div class="col-3 text-center">
            <div>pozostali: <div class="text-bold" style="border: solid 3px black">{{
              viewCurrency(sum(notMemberCostSum))
                }}
              </div>
            </div>
            <div>po rabacie: <div class="text-bold text-primary" style="border: solid 3px black">{{
        ...
        <div class="col">
          <q-card-actions class="row" align="right">
            <q-item>
              <q-btn class="full-width col" color="primary" icon="close" v-close-popup/>
            </q-item>
            <q-item>
              <q-btn class="full-width col" color="primary" :loading="loading[0]" icon="done"
                :disable="dis || memberName.legitimationNumber === 0 || otherName.id === 0"
                @click="dis = true; simulateProgress(memberName.legitimationNumber, otherName.id)"/>
            </q-item>
          </q-card-actions>
        </div>
      </q-card>
...
export default {
  ...
  setup () {
    ...
    function simulateProgress (memberNumberLegitimation, otherID) {
