        ...
        <q-card-section class="col-3">
          <q-item class="col">
            <q-input class="full-width" bg-color="primary" standout="" rounded input-class="text-positive"
              label-color="positive" dense v-model="firstDateAmmo" mask="####-##-##" label="Data początkowa">
              <template v-slot:append>
                <q-icon name="event" color="positive" class="cursor-pointer">
                  <q-popup-proxy ref="qDateProxy" transition-show="scale" transition-hide="scale">
                    <q-date @input="memberAmmoTakesInTime()" v-model="firstDateAmmo" mask="YYYY-MM-DD"
          ...
          <q-item class="col">
            <q-input class="full-width" bg-color="secondary" standout="" rounded input-class="text-positive"
              label-color="positive" dense v-model="secondDateAmmo" mask="####-##-##" label="Data końcowa">
              <template v-slot:append>
                <q-icon name="event" color="positive" class="cursor-pointer">
                  <q-popup-proxy ref="qDateProxy" transition-show="scale" transition-hide="scale">
                    <q-date @input="memberAmmoTakesInTime()" v-model="secondDateAmmo" mask="YYYY-MM-DD"
                      class="bg-dark text-positive">
                      <div class="row items-center justify-end">
                        <q-btn v-close-popup label="Zamknij" color="primary" />
                      </div>
...
export default {
  ...
  methods: {
    ...
    memberAmmoTakesInTime () {
      fetch(`${this.local}/statistics/memberAmmoTakesInTime?firstDate=${this.firstDateAmmo.replace(/\//gi, '-')}&secondDate=${this.secondDateAmmo.replace(/\//gi, '-')}`, {
        method: 'GET'
      }).then(response => {
