  ...
  <div class="bg-dark">
    ...
    <q-card class="row bg-dark text-positive">
      <q-card-section :class="backgroundDark ? 'col-6 bg-dark' : 'col-6 bg-grey-3'">
        ...
        <q-input v-model="userSecondName" input-class="text-positive" label-color="positive" class="full-width" dense
          ...
          onkeypress="return (event.charCode > 64 && event.charCode < 91) || (event.charCode > 96 && event.charCode < 123) || (event.charCode > 210 && event.charCode < 400) || event.charCode == 32"
          filled />
        <q-select class="full-width" rounded bg-color="primary" standout="" multiple v-model="userSubTypeSelect" dense options-dense fill-input
          label-color="positive" color="positive" input-class="text-positive" emit-value
      ...
      <q-card-section class="col-6 text-positive">
        ...
        <ol>
          <li v-for="(user, id) in users.filter(f=> f.userPermissionsList.toString().includes('Super Użytkownik'))" :key="id" class="col text-bold rounded">
            <div @click.ctrl="uuid = user.uuid; deleteUser = true" @dblclick="uuid = user.uuid; inputBarCode = true" class="row full-width flex-center bg-grey-3 q-ma-sm">
    ...
    <q-dialog v-model="inputBarCode">
      <q-card class="text-center bg-dark" style="min-width: 30vw">
        <q-card-section>
          ...
          <q-select class="full-width" rounded bg-color="primary" standout="" multiple v-model="userSubTypeSelect1" dense options-dense fill-input
          label-color="positive" color="positive" input-class="text-positive" emit-value
          map-options popup-content-class="bg-dark text-positive" options-selected-class="bg-dark text-positive"
          :options="permissions" label="Wybierz uprawnienia">
          <template v-slot:no-option>
    ...
    <q-dialog v-model="acceptCodeUser2" persistent @keypress.enter="editUser(); acceptCodeUser2 = false; code = null">
      <q-card class="bg-red-5 text-center">
        ...
        <q-card-actions align="right">
          <q-btn label="anuluj" color="black" v-close-popup @click="code = null" />
          <q-btn label="Dodaj" color="black" v-close-popup
...
export default {
  ...
  methods: {
    ...
    deleteUserFunction (uuid, code) {
      fetch(`${this.local}/users/delete?userID=${uuid}&code=${code}`, {
        method: 'DELETE'
      }).then(response => {
