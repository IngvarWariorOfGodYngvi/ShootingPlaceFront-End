<template>
  <div v-if="member != null" class="full-width">
    ...
    <div class="text-h6 text-center bg-warning" v-if="shootingPlace === 'prod' && !member.declarationLOK">Brak Podpisanej
      Deklaracji LOK</div>
    <q-card bordered class="container round" :class="`row bg-${member.active ? 'green' : 'red'}-3`"
    ...
    <q-card bordered :style="`rotate: ${rotateFun}deg;`" :class="mobile ? 'col bg-dark' : 'row bg-dark'">
      ...
      <q-card-section class="col-5 text-center">
          ...
          <div>
            ...
            <q-card flat class="row bg-dark">
              <q-card-section class="col q-pa-none">
                <div v-for="(calibers, uuid) in calibers" :key="uuid" class="q-pa-xs hover">
                  <q-radio dense v-model="caliberUUID" :val="calibers.uuid" class="full-width text-left"
                    :label="calibers.name">
                  </q-radio>
                </div>
              </q-card-section>
            ...
            <div v-if="!member.erased" class="row">
              <q-input dense
                ...
                type="number"></q-input>
              <q-btn dense class="full-width col round" :disable="caliberUUID == null || quantity.length === 0"
        ...
        <q-expansion-item icon dense v-if="!member.erased && main && !mobile"
          :header-class="!headerColorChange ? '' : 'bg-grey-3 text-black round'"
          ...
          <template v-slot:header>
            <q-item-section
              v-if="(!member.pzss && member.club.id == 1) || (!member.declarationLOK && shootingPlace === 'prod')"
          ...
          <q-expansion-item dense v-if="member.adult && !member.erased"
            ...
            <div class="row">
              ...
              <q-btn dense class="col round" v-if="member.weaponPermission.exist" label="Usuń pozwolenie"
          ...
          <q-expansion-item dense v-if="member.adult && !member.erased"
            ...
            <div class="row">
              ...
              <q-btn dense class="col round" v-if="!member.weaponPermission.admissionToPossessAWeaponIsExist"
          ...
          <q-expansion-item dense v-if="member.adult && !member.erased" label="Uprawnienia"
            ...
            <q-expansion-item dense v-if="member.adult && !member.erased"
              ...
              <q-item class="full-width">
                ...
                <q-btn dense class="col full-width round" v-if="member.memberPermissions.arbiterNumber != null"
              ...
              <div class="col">
                ...
                <q-item class="full-width">
                  <q-btn dense class="full-width col-1 round" v-if="member.memberPermissions.arbiterNumber !== null"
          ...
          <q-expansion-item dense v-if="!member.erased" label="Portal PZSS" group="right-right-card">
            ...
            <div class="row">
              <q-tooltip content-class="bg-dark text-positive" anchor="top middle" :delay="750">kliknij aby
                edytować</q-tooltip>
              <q-btn dense class="col q-ma-md round" text-color="black"
          ...
          <q-expansion-item dense v-if="!member.erased && shootingPlace === 'prod'" label="Deklaracja LOK"
            ...
            <div class="row">
              <q-btn dense class="col q-ma-md" text-color="black"
          ...
          <q-expansion-item dense v-if="!member.adult" label="Przenieś do Grupy Ogólnej" group="right-right-card">
            ...
            <q-item>
              <q-btn :disable="!checkInternship(member.joinDate) || !member.active || !checkAge(member.pesel)"
          ...
          <q-expansion-item dense label="Zmień klub macierzysty" @click=" getAllClubs()" class="text-center">
            <div class="text-left">
              ...
              <q-btn dense v-if="clubChoice !== '' && clubChoiceToggle" class="full-width round" color="primary"
        ...
        <q-expansion-item dense label="Historia startów" group="right-card"
          ...
          @show="headerColorChange1 = true" class="text-positive" default-opened>
          <div class="q-pa-xs">
      ...
      <q-card bordered :class="mobile ? 'col bg-dark text-positive' : 'row full-width bg-dark text-positive'">
        <q-card-section :class="`col-${main && !mobile ? '3' : '6'}`" style="height:100%;">
          <div class="full-height text-positive">
            ...
            <div class="rounded">
              <q-btn class="full-width" square v-if="!member.erased && !mobile" label="Zmień Dane Podstawowe"
                color="secondary"
          ...
          <div class="full-height text-positive">
            <div style="height: 55%">
              ...
              <div class="row items-center"><q-item-label class="text-positive" caption lines="2">e-mail: {{
                  member.email
                  }}</q-item-label><q-btn icon="content_copy" size="0.5em" @click="copyEmail(member.email)"></q-btn>
              </div>
            ...
            <div class="rounded">
              <q-btn v-if="!member.erased && !mobile" class="full-width" label="Zmień Dane Kontaktowe"
                color="secondary"
    ...
    <q-dialog v-model="contributionCode" persistent>
      <q-card class="bg-green-5 text-center">
        ...
        <q-card-actions align="right">
          ...
          <q-btn label="Przedłuż" color="black" v-close-popup @click=" simulateProgress(); code = null" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="addressConfirm1">
      <q-card class="bg-dark text-positive">
        ...
        <q-card-actions align="right">
          <q-btn label="anuluj" color="secondary" v-close-popup />
          <q-btn label="zmień" color="primary" v-close-popup @click=" updateMemberCode = true" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    <q-dialog v-model="updateMemberCode" persistent>
      <q-card class="bg-red-5 text-center">
        ...
        <q-card-actions align="right">
          <q-btn label="anuluj" color="black" v-close-popup @click=" code = null" />
          <q-btn label="Przenieś" color="black" v-close-popup
    ...
    <q-dialog v-model="basicDataConfirm1">
      <q-card class="bg-dark text-positive">
        ...
        <q-card-actions align="center">
          <q-btn label="anuluj" color="secondary" v-close-popup />
          <q-btn label="zapisz" color="primary" v-close-popup @click=" updateMemberCode = true" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="addAmmoConfirm" @keypress.enter="showloading();simulateProgress1(); addAmmoConfirm = false">
      <q-card class="bg-dark text-positive">
        ...
        <q-card-actions align="center">
          <q-btn label="anuluj" color="secondary" v-close-popup />
          <q-btn label="wydaj" color="primary" v-close-popup @click="showloading(); simulateProgress1()" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="showCompetition">
      <q-card class="text-center bg-dark text-positive" style="min-width: 60vw">
        <q-card-section class="row text-center">
          <div class="text-h5 text-bold text-center full-width">Starty zawodnika w {{ tournament.name }} z
            {{ tournament.date }}
          </div>
